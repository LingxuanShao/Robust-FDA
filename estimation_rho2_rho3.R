...
obtainhatmurobust=function(s,n,m,t,X,hmu,robu){
  ...
  while((maxX-minX)>0.01){#be careful with this
...
###  main
#settings
for(robucase in c(1:7)){
  if(robucase==1){robu=1;kappa=0.001;}
  if(robucase==2){robu=1;kappa=0.01;}
  if(robucase==3){robu=1;kappa=0.1;}
  if(robucase==4){robu=1;kappa=1;}
  if(robucase==5){robu=2;}
  if(robucase==6){robu=3;}
  if(robucase==7){robu=0;}
...
cat(robu,kappa,scorenumber,n,m,mean(itresult_mu_square),sd(itresult_mu_square)/sqrt(itermax),mean(itresultCrela),sd(itresultCrela)/sqrt(itermax),"\n")
output=c(robu,kappa,scorenumber,n,m,mean(itresult_mu_square),sd(itresult_mu_square)/sqrt(itermax),mean(itresultCrela),sd(itresultCrela)/sqrt(itermax))
write.csv(output,paste0("~/",robu,"_",kappa,"_",scorenumber,"_",n,"_",m,sep=""));
}}}
